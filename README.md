...
## 일지 보기 / 문서 탭 UI 개선 사용법 (P62 · P62-1)
...
| **필터** | 태그/기간 · 통계는 「통계 보기」 |
| **정렬** | 최신순 / 오래된순 (드로어) |
| **카드** | 최대 120px · 빈 상태 CTA 「첫 일지 작성하기」 |

1. 일지 탭 → **보기**
2. **날짜**로 일 선택 · 카드 클릭으로 **작성** 이동
3. 필터·정렬은 상단 버튼 → 드로어(모바일은 하단 시트)

### 문서 · 작성 | 보기

| 탭 | 내용 |
|----|------|
| **작성** (`#write`) | 문서 선택기 · 제목+마크다운 · 미리보기 · 카테고리/저장/내보내기 · 3초 자동저장 |
| **보기** (`#view`) | 검색+칩(전체/내 문서/공유/북마크) · 폴더 2depth · 카드 그리드 · 필터/정렬 드로어 |

1. 문서 탭 상단 **작성 | 보기** 전환
2. 보기에서 카드 클릭 → 작성 탭으로 해당 문서 오픈
3. 작성 중 dirty 상태에서 보기 전환 시 저장 확인
...
일지 탭에서 **작성**과 **보기**를 분리해, 에디터와 탐색/집계를 나란히 씁니다.

### 작성 / 보기 탭

| 탭 | 내용 |
|----|------|
| **작성** | Writing-first 단일 에디터 (통합 툴바 · 하단 태그/주석/필드) |
| **보기** | 트리 · 캘린더 · 목록 · 통계 하위 탭 |

보기에서 날짜를 고르면 **작성** 탭으로 이동해 해당 일지를 엽니다.

### 트리 폴더

1. 보기 → **트리** — 좌측 폴더 트리 · 우측 일지 목록
2. 기본 폴더: **미분류 / 날짜별 / 프로젝트별 / 태그별**
3. **+** 또는 우클릭으로 커스텀 폴더 생성 · 이름변경 · 삭제
4. 일지를 폴더로 **드래그**해 이동 (같은 일지를 여러 폴더에 참조 가능)
5. 폴더 선택 시 하위 일지만 목록에 필터

### 캘린더

1. 보기 → **캘린더** · 월간 / 주간 / 일간 전환
2. **파랑** = 작성 있음 · **회색** = 없음 · **노란 링** = 오늘
3. 날짜 클릭 → 작성 탭으로 이동
4. 작성된 날을 다른 날로 **드래그**하면 일지 날짜 키 이동 (대상에 본문이 있으면 거부)

### 목록 · bulk

1. 보기 → **목록** — 시간순 · 첫 100자 미리보기
2. 검색 · 태그 · 날짜 범위 · 폴더 필터
3. 체크박스 · **Shift+클릭**으로 범위 선택
4. 선택 후 **이동 / 태그 / 내보내기 / 삭제**

### 통계

1. 보기 → **통계**
2. 기간(1주·1개월·3개월·전체)별 작성 추이 · 태그 빈도
3. **연속 작성(streak)** · **작성 시간대 히트맵**

### 경로 딥링크

```
/journal/{folderSlug}/{YYYY-MM-DD}
→ /?tab=journal&date=…&folder=…
```

예: `/journal/uncategorized/2026-07-31`

## UI/UX 개선 사용법 (Phase 30)

Phase 30 마감 시 반영된 UI/UX 5건입니다.

### 1. 작성 / 보기 분리

| 탭 | 역할 |
|----|------|
| **작성** | 일지 에디터만 — 날짜 이동 · 저장 · 내보내기 · Obsidian |
| **보기** | 트리 · 캘린더 · 목록 · 통계 |

보기에서 날짜를 고르면 작성 탭으로 이동합니다.

### 2. 요약 사이드바

우측 사이드바는 **핵심 위젯**만 먼저 보여 주고, **더보기**로 추가 위젯·시스템 도구를 펼칩니다.

### 3. 일지 에디터

- **상단 툴바**: 이전/다음 날짜 · 저장(44px+) · 내보내기 · Obsidian 가져오기
- **하단 패널**: 태그 · 주석 · 커스텀 필드를 한 줄(넓은 화면 3열)로 배치
- 툴바/하단은 본문과 배경색으로 구분됩니다

### 4. 웰컴 모달

- 첫 방문 시만 표시 (`localStorage` 키 `welcome_seen`)
- 일지 / 문서 / 일정 세 가지를 한 화면에서 소개 · **시작하기**
- **다시 보지 않기** 체크 · 상단 닫기 · 모바일은 전체 화면 · blur/fade

### 5. 버튼 · 터치 타겟

- 인터랙티브 요소 **최소 44×44** · 버튼 간격 **8px+**
- `focus-visible` 2px ring · primary `slate-900` · secondary `slate-100` · danger `red-600`
- `rounded-lg` · `shadow-sm` 통일 (`src/components/ui/button.tsx`)
...
사이드바 **AI** 버튼. 상세: [docs/AI.md](docs/AI.md)

1. **작성** — 문장 제안 · 완성 · 태그 · 요약 · 키워드 · 관련 문서/일지 추천
2. **편집** — 선택 요약/확장/재작성 · 문법 · 번역(ko/en/ja)
3. **의미검색** — 자연어 질문 · 로컬 임베딩 · 관련성 순
4. **분석** — 감정 · 키워드 트렌드 · 프로젝트 진행 요약

환경변수: `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` / `GEMINI_API_KEY` · `FOLIO_AI_MODEL`(gpt-4, claude-3, gemini-pro 등). 키 없으면 로컬 폴백.

고급 검색 **의미 검색** 체크 → Lunr + 임베딩 하이브리드.
