...
> 프로세스 화면의 Provider 카드에서 `이 기기`와 `Beacon 확장`을 전환할 수 있으며,
> 명시적인 선택은 브라우저에 기억된다.
> 로컬 Provider와 로컬 저장 모드에서는 Beacon API를 백그라운드에서 점검하지 않는다.
> 연결 메뉴를 열거나 Beacon을 실제 선택한 경우에만 가용성을 확인한다.
> 연결 실패 시 Folio는 로컬 Provider로 안전하게 폴백하며, 로컬 데이터와 Beacon 연결
> 설정을 서로 덮어쓰지 않는다. 사용자는 화면에서 재시도하거나 로컬 사용을 확정할 수 있다.
