...
Folio는 브라우저에 저장되는 개인·팀 워크스페이스다. 일지 · 문서 · 칸반 · 설치가 필요 없는 로컬 프로세스 탭을 제공한다.
...
4. **프로세스** — 로컬 Gate / Timeline을 바로 편집·저장하고, 필요하면 Beacon 확장 연결
5. **저장 모드** — 사이드바에서 `로컬` / `클라우드` 전환, Beacon은 고급 확장에서 선택
