...
버전 업그레이드 · 마이그레이션 · 다운타임 최소화. (P22 · **v4.2 갱신**)
...
| 구분 | 예 | 의미 |
|------|----|------|
| 정식 | `4.2.0` | Phase 40 · P68 Local-first Process 완료 |
| Patch | `4.2.x` | 버그픽스 · 문서 · 호환성 업데이트 |

체크리스트:

- [x] Beacon 없이 프로세스 생성·편집·저장
- [x] Local/Beacon Provider 전환 시 데이터 보존
- [x] Beacon 연결 실패 폴백·재시도 UX
- [x] lint · typecheck · 전체 271개 테스트 · production build
...
| 구분 | 예 | 의미 |
|------|----|------|
| 정식 | `3.5.0` | Phase 33 완료 릴리즈 (알림/메시지) |
| Patch | `3.5.1` | 버그픽스 · 문서 |
| WIP | `3.x.0-wip` | 다음 Phase 진행 중 |
...
- [ ] `npm run runbook:backup`
- [ ] 알림 허브 · 메시지 · 이메일 outbox/Resend · rich 푸시 스모크 ([docs/NOTIFICATIONS.md](../NOTIFICATIONS.md))
- [ ] `npm ci` · `npm run lint && npm run typecheck && npm run test && npm run qa:smoke`
- [ ] Preview/스테이징 스모크 후 Production
...
| 구분 | 예 | 의미 |
|------|----|------|
| 정식 | `3.3.0` | Phase 31 완료 릴리즈 (문서 버전 관리) |
| WIP | `3.3.0-wip` | Phase 진행 중 |
| Patch | `3.3.1` | 버그픽스 · 문서 |

체크리스트:

- [ ] `npm run runbook:backup`
- [ ] 문서 탭 버전 이력 · Diff · 복원/체크아웃 스모크 ([docs/DOC-VERSIONS.md](../DOC-VERSIONS.md))
- [ ] `npm ci` · `npm run lint && npm run typecheck && npm run test && npm run qa:smoke`
- [ ] Preview/스테이징 스모크 후 Production
...
| 구분 | 예 | 의미 |
|------|----|------|
| 정식 | `3.2.0` | Phase 30 완료 릴리즈 (일지 트리·캘린더 · UI/UX) |
| 정식 | `3.0.0` | Phase 28 완료 릴리즈 |
| WIP | `3.x.0-wip` | Phase 진행 중 |
| Patch | `3.2.1` | 버그픽스 · 문서 |

체크리스트:

- [ ] `npm run runbook:backup`
- [ ] Quick Capture · 템플릿 · 타이머 · 북마크 스모크
- [ ] `npm ci` · `npm run lint && npm run typecheck && npm run test && npm run qa:smoke`
- [ ] Preview/스테이징 스모크 후 Production
